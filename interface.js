(function(window, document){
    ...
    Interface.prototype.click = function(name) {
        // TODO: move "ct" param here from html file
        name = name || "default";
        if (this.customParams["cf"] === 0 || Math.round(this.timeData.currentTime) >= this.customParams["cf"]) {
            if (this.customParams["plc"]) { // if only player should open url
                sendToAPP("action", {type: "AdClickThru", id: name, url: getClickURL(this.clickUrl, name)}, this.id);
            } else {
                window.open(getClickURL(this.clickUrl, name));
                sendToAPP("action", {type: "AdClickThru", id: name}, this.id);
            }
            CustomEvent.trackEvent("AdClickThru");
        } else {
            // click disabled by ClickFrom custom parameter
        }
    ...
    function getClickURL(url, name) {
        if(name && name !== "default") {
            //var ids = name.match(/[0-9]$/);
            var ids = name.match(/\d+/g);
            if(ids && ids.length == 1) {
